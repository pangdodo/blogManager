...
<div id="addbaominformdivid" hidden="" class="layui-fluid" style="margin: 15px;">
...
<script type="text/javascript">
    ...
    layui.use(['layer','form','layedit','laydate'], function(){
        ...
        //监听提交，创建比赛项目表单提交在此操作（xumin 2018.11.17）
        form.on('submit(addeditsubmitfilter)', function(data) {
            ...
            $.ajax({
                type: "POST",
                url:"badminton_s/addupdatebatminton",
      ...
      //监听提交，羽毛球报名表单提交在此操作（xumin 2018.11.17）
        ...
        $("#edit").click(function () {   //编辑比赛内容   xumin 2018年11月18
            ...
            if (id) {
                ...
                $.ajax({
                    type: "POST",
                    url:"badminton_s/selectBadmintonById",
                    data:{id:ret.id},
                    async: false,
                    error: function(request) {
                        layer.alert("与服务器连接失败/(ㄒoㄒ)/~~");
            ...
            jQuery("#list2").jqGrid(
                    {
                        ...
                        url: 'wx/lists',  //xumin  2018年11月18日修改  ,显示比赛项目列表
                        //url : 'static/jqgrid/data/JSONData.json',//组件创建完成之后请求数据的url
                        //styleUI: 'Bootstrap',
                        datatype: "json",//请求数据返回的类型。可选json,xml,txt
                        emptyrecords: "当前无记录",
                        colNames: [ '比赛项目名称'],//jqGrid的列显示名字
                        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
                           // {name: 'id', index: 'id', width: 10, sortable: true, search: false},
                            {name: 'game_name', index: 'game_name', width: 100, sortable: false,search: true,
                                //被该列搜索时的搜索条件有哪些
       ...
       function baomin(id)
        {
          ...
          document.getElementById("Badminton_id").value=id;
